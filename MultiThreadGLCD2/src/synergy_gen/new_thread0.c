...
#endif
static uint8_t new_thread0_stack[1024] BSP_PLACE_IN_SECTION(".stack.new_thread0") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
TX_MUTEX glcdMutex;
...
void new_thread0_create(void)
{
    ...
    tx_mutex_create (&glcdMutex, (CHAR *) "GLCD Mutex", TX_NO_INHERIT);

    tx_thread_create (&new_thread0, (CHAR *) "New Thread", new_thread0_func, (ULONG) NULL, &new_thread0_stack, 1024, 1,
                      1, 1, TX_AUTO_START);
}
